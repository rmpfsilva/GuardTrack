...
- **Stripe Connect (Dormant)**: Infrastructure for Stripe Connect payment processing. Companies connect Stripe accounts (`companies.stripeAccountId`), guards connect payout accounts (`users.stripeConnectedAccountId`). Currently dormant — buttons disabled until platform Stripe keys are configured post-registration. Status endpoints: `/api/stripe/connect/company/status`, `/api/stripe/connect/guard/status`. Payment service in `server/stripe.ts`.
